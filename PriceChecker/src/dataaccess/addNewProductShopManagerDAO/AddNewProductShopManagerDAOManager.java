 ...
 * @author Piotr
 */
public class AddNewProductShopManagerDAOManager
    implements AddNewProductShopManagerDAO
{
  ...
  @Override public String addNewProduct(String clientUsername,
      String productName, String productDescription, String category,
      ArrayList<String> parseTag, int price) throws SQLException
  {
    ...
    try (Connection connection = databaseConnection.getConnection())
    {
      ...
      if (!(product == null))
      {
        String response = checkIfInPriceTable(product.getProductId(), price,
            clientUsername);
      ...
      else
      {
        if (price > 0)
        {
          int id = addToTheProductTable(productName, productDescription,
              category);
  ...
  private String checkIfInPriceTable(int productId, int price,
      String clientUsername) throws SQLException
  {
    try (Connection connection = databaseConnection.getConnection())
    {
      PreparedStatement statement = connection.prepareStatement(
          "SELECT productid, price, price.userid\n" + "FROM price\n"
              + "INNER JOIN users\n"
  ...
  private void updatePriceInTheTable(int userId, int price) throws SQLException
  {
    ...
    {
      PreparedStatement statement = connection
          .prepareStatement("UPDATE price set price = ? where userid = ? ");
  ...
  private void addThePriceToTheProduct(int userId, int productId, int price)
      throws SQLException
  {
    try (Connection connection = databaseConnection.getConnection())
    {
      PreparedStatement statement = connection.prepareStatement(
          "INSERT into price(productid, price, userid) values" + " (?,?,?) ");
  ...
  {
    {
      ...
      try
      {
        ...
        statement = connection
            .prepareStatement("SELECT * FROM category where categoryname = ?");
  ...
  @Override public String addNewTag(String newTag) throws SQLException
  {
    ...
    {
      PreparedStatement statement = connection
          .prepareStatement("SELECT * FROM tag where tagname = ?");
  ...
  @Override public ArrayList<Product> getAllProductsFor(String username)
      throws SQLException
  {
    {
      try (Connection connection = databaseConnection.getConnection())
      {
        PreparedStatement statement = connection.prepareStatement(
            "SELECT product.productid, product.productname, product.productdescription, "
                + "product.categoryname, price.price "
                + "from price inner join product on product.productid = "
  ...
  @Override public String editNewProduct(int userId, int price, int productid)
      throws SQLException
  {
    {
      if (price > 0)
      {
        ...
        {
          PreparedStatement statement = connection.prepareStatement(
              "INSERT INTO  price(userId, price, productId) VALUES (?,?,?)");
      ...
      throws SQLException
  {
    ...
    {
      PreparedStatement statement = connection
          .prepareStatement("select * from producttag where productid =?");
      statement.setInt(1, productId);
      ResultSet resultSet = statement.executeQuery();
      ArrayList<String> tags = new ArrayList<>();
      while (resultSet.next())
  ...
  @Override public int getUserId(String username) throws SQLException
  {
    try (Connection connection = databaseConnection.getConnection())
    {
      PreparedStatement statement = connection
  ...
  private void addToTheProductTagTable(int id, ArrayList<String> parseTag)
      throws SQLException
  {
    try (Connection connection = databaseConnection.getConnection())
    {
      ...
      {
        PreparedStatement statement = connection.prepareStatement(
            "INSERT INTO producttag(productid, tagname) VALUES(?,?)");
  ...
  private int addToTheProductTable(String productName,
  ...
  private void addNewTagNow(String newTag) throws SQLException
  {
    ...
    {
      PreparedStatement statement = connection
          .prepareStatement("INSERT INTO tag (tagname) VALUES(?)");
      statement.setString(1, newTag);
      statement.executeUpdate();
    }
  }
